...
public:
    ...
    inline T& set(uint8_t i, uint8_t j){return _data[index(i,j)];}; // sets one single element
    inline T& operator()(uint8_t i, uint8_t j=0){return _data[index(i,j)];}; //sets and returns one single element
    ...
    inline MatrixX<T>& operator=(const MatrixX<T> &rhs){return copyMatrix(rhs);}; // assignment
    ...
    //==================Matrix Data==================//
    inline T& get(uint8_t i, uint8_t j) const{return _data[index(i,j)];}; //retrieves one single element
    T trace() const; // retrieves the trace
    inline uint8_t rows() const {return _nrows;}; // retrieves the number of rows
    inline uint8_t columns() const{return _ncols;}; // retrieves the number of columns
    inline uint8_t length() const{return fmax(_nrows, _ncols);}; // retrieves the lenght of the Matrix
    ...
    //===============Auxiliary Functions==============//
    template <typename T2> inline MatrixX<T>& copyData(const T2* data){
        for (uint8_t i=0; i<_nrows*_ncols; i++)
        this->_data[i] = (T) data[i];
        return *this;}; //sets new data
    inline T& getData(uint8_t i) const{return _data[i];}; //retrieve a single data
    inline T* data() const{return this->_data;}; // retrieve all data
...
private:
    ...
    inline uint8_t index(uint8_t i, uint8_t j) const{return (i * _ncols + j);};
    //-------Private Variables-----//
    T* _data;
    uint8_t _nrows;
    uint8_t _ncols;
